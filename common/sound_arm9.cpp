...
namespace NDS_Sound
{
    ...
    {
        fifoSendValue32(FIFO_USER_01, SOUND_MASTER_ENABLE);
    }
    void soundDisable(void)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_MASTER_DISABLE);
    }
    ...
    {
        fifoSendValue32(FIFO_USER_01, SOUND_PAUSE | (soundId << 16));
    }

    void soundKill(int soundId)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_KILL | (soundId << 16));
    }

    void soundResume(int soundId)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_RESUME | (soundId << 16));
    }

    void soundSetVolume(int soundId, u8 volume)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_SET_VOLUME | (soundId << 16) | volume);
    }

    void soundSetPan(int soundId, u8 pan)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_SET_PAN | (soundId << 16) | pan);
    }

    void soundSetFreq(int soundId, u16 freq)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_SET_FREQ | (soundId << 16) | freq);
    }

    void soundSetWaveDuty(int soundId, DutyCycle cycle)
    {
        fifoSendValue32(FIFO_USER_01, SOUND_SET_WAVEDUTY | (soundId << 16) | cycle);
    }
    ...
    int soundMicRecord(void* buffer, u32 bufferLength, MicFormat format, int freq, MicCallback callback)
    {
        ...
        fifoSetDatamsgHandler(FIFO_USER_01, micBufferHandler, 0);
    ...
    {
        fifoSendValue32(FIFO_USER_01, MIC_STOP);
    }
} // namespace NDS_Sound
