...
void* Alloc(size_t bytes_to_alloc, MemoryFlagType flags)
{
    ...
    if (!mem_ptr) {
      DBG_LOG("Malloc returned NULL: %lu bytes requested.", bytes_to_alloc);
    }
...
#if !defined(__APPLE__) && !defined(__unix__) && !defined(__unix) && !defined(_WIN32) && !defined(_NDS) && !defined (_N64)
...
size_t Ram_Free(void)
{
    ...
    size_t heap_end = (size_t)HEAP_START_ADDR;
    size_t heap_top = (size_t)KSEG0_START_ADDR + get_memory_size() - 0x10000;
    size_t heap_size = heap_top - heap_end;
    size_t m = (heap_size - mi.arena) + mi.fordblks;
    DBG_LOG("RAM avaliable: %ldkb\n", m / 1024);
/*
           DBG_LOG("Total non-mmapped bytes (arena):       %u", mi.arena);
           DBG_LOG("# of free chunks (ordblks):            %u", mi.ordblks);
           DBG_LOG("# of free fastbin blocks (smblks):     %u", mi.smblks);
           DBG_LOG("# of mapped regions (hblks):           %u", mi.hblks);
           DBG_LOG("Bytes in mapped regions (hblkhd):      %u", mi.hblkhd);
           DBG_LOG("Max. total allocated space (usmblks):  %u", mi.usmblks);
           DBG_LOG("Free bytes held in fastbins (fsmblks): %u", mi.fsmblks);
           DBG_LOG("Total allocated space (uordblks):      %u", mi.uordblks);
           DBG_LOG("Total free space (fordblks):           %u", mi.fordblks);
           DBG_LOG("Topmost releasable block (keepcost):   %u", mi.keepcost);
*/
    return m;
#else
