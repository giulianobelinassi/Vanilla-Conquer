...
void* Extract_Shape(void const* buffer, int shape)
{
    ...
    uint16_t numshapes;
    memcpy(&numshapes, buffer + offsetof(ShapeBlock_Type, NumShapes), sizeof(short));

    /*
    ----------------------- Return if invalid argument -----------------------
    */
    if (!buffer || shape < 0 || shape >= numshapes)
        return (NULL);

    /*  Same as offset = block->Offsets[shape]; on arch that unaligned access
        behaves well.  */
    memcpy(&offset, buffer + offsetof(ShapeBlock_Type, Offsets) + shape * sizeof(uint32_t), sizeof(uint32_t));
