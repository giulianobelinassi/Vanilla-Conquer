    ...
    for (StructType sindex = STRUCT_FIRST; sindex < STRUCT_COUNT; sindex++) {
        ...
        */
        if (building.IsBuildable) {
            ...
            ((void const*&)building.CameoData) = Get_Shape_Pointer(fullname);
        }
        ...
        */
        ...
        _makepath(fullname, NULL, NULL, buffer, ".SHP");
        void const* dataptr = Get_Shape_Pointer(fullname);
        ...
        */
        ...
        ((void const*&)building.ImageData) = Get_Shape_Pointer(fullname);
    }

    // Try to load weap2.shp
    char fullname[_MAX_FNAME + _MAX_EXT];
    _makepath(fullname, NULL, NULL, (char const*)"WEAP2", ".SHP");
    WarFactoryOverlay = Get_Shape_Pointer(fullname);
    ...
    if (theater != LastTheater) {
        ...
        for (StructType sindex = STRUCT_FIRST; sindex < STRUCT_COUNT; sindex++) {
            ...
            if (classptr->IsTheater) {
                ...
                ((void const*&)classptr->ImageData) = Get_Shape_Pointer(fullname);
            }
