...
When a guest changes color, he asks the host for the new color, and assumes the request will go through. If it doesn't,
the host sends the guest a messages setting him back to the old color, otherwise, everyone gets the new color for the
guest. When the host changes a game param, the change will be noticed within PARAMREFRESHWAIT milliseconds and then
transmitted to all guests. All player specific information is stored within the players list (IconListClass), and
nowhere else. Though this storage method got a bit inelegant, it gets the job done. All "informs" (messages sent from
host to guest) except color changes (WOL_GAMEOPT_INFCOLOR) and player "accept" status changes (WOL_GAMEOPT_INFACCEPT)
are assigned a ID, which increments on each send. Color informs are: a) not a condition for removing a guest's "accept"
status, and b) sent out to individual guests in certain cases, and would thus complicate the ID tracking scheme. When a
guest "accepts" the game setup (with a WOL_GAMEOPT_REQACCEPT), the latest received ID is included in the message. If the
host receives a WOL_GAMEOPT_REQACCEPT with an out-of-date ID, the accept is ignored. Though the guest sets himself to
"accepted" when he sends the WOL_GAMEOPT_REQACCEPT, if it gets ignored by the host it is because a new inform is already
on its way, which will reset the guest's own status to "not accepted" when it arrives. Host clears all accepteds when
params are changed, player joins or leaves, or house change message arrives. Though a WOL_GAMEOPT_INFACCEPT is sent out
when a guest accepts, there is no corresponding "unaccept all" message sent out at this point, as all guests will
naturally clear their recorded accept status for everyone when the change arrives there. Guest clears own accepted when
an inform from the server other than color change arrives, player joins or leaves, or own house is directly changed.
When all players are "accepted", and the host says "start", a WOL_GAMEOPT_INFSTART is sent to all guests and the host
goes into "waiting to start" mode. If a house change or other arrives in this phase, host clears waiting mode and
(naturally) marks guests unaccepted and sends out the house change to the guests. (The same applies to player
joins/leaves.) While in this mode, the host user is locked out of making any changes to game params. If a change does
come in during this phase, a WOL_GAMEOPT_INFCANCELSTART is sent to the guests, and the waiting mode cancelled. When a
guest receives WOL_GAMEOPT_INFSTART: If he does not see himself as accepted, it is because a change has just been sent
to the host, or some other event occurred that the host will learn about, so the guest can simply ignore the
WOL_GAMEOPT_INFSTART. If the guest does see himself as accepted when WOL_GAMEOPT_INFSTART arrives, he responds with
WOL_GAMEOPT_REQSTART and goes into "waiting to start" mode. When in this mode, the user is locked out of making any
changes. Note that the host is not really asking for any sort of "confirmation that it's ok" to start from the guest
here. (Hence no "cancel" response is possible.) It's really just a way of telling the guests not to make any more
changes to the setup; it's like we're making sure we've "flushed the queue" of game changes. If a guest receives a
WOL_GAMEOPT_INFCANCELSTART, he cancels out of "waiting to start" mode. Note that while in the waiting mode, normal
processing of messages is allowed to take place - the user is simply blocked from doing anything. Presumably (hopefully)
this phase will last less than a second. When the host receives a WOL_GAMEOPT_REQSTART from everyone, he tells everyone
to start with a WOL_GAMEOPT_INFGO. Because there is a chance of color changes being out of sync, all player colors are
sent to all guests at this point. There is no chance of other data being out of sync.
*/
...
void WOL_GameSetupDialog::Initialize()
{
    ...
    */
    ...
    pILPlayers = new IconListClass(BUTTON_PLAYERLIST,
                                   d_playerlist_x,
                                   d_playerlist_y,
                                   d_playerlist_w,
                                   d_playerlist_h,
    ...
    pILScens = new IconListClass(BUTTON_SCENARIOLIST,
                                 d_scenariolist_x,
                                 d_scenariolist_y,
                                 d_scenariolist_w,
                                 d_scenariolist_h,
                                 ...
                                 true,
                                 1);
    pILDisc = new IconListClass(BUTTON_DISCLIST,
                                d_disc_x,
                                d_disc_y,
                                d_disc_w,
                                d_disc_h,
    ...
    pEditSend = new EditClass(
        BUTTON_SENDEDIT, szSendBuffer, MAXCHATSENDLENGTH, TPF_TEXT, d_send_x, d_send_y, d_send_w, d_send_h);
    ...
    pCheckListOptions = new CheckListClass(BUTTON_PARAMS,
                                           d_options_x,
                                           d_options_y,
                                           d_options_w,
                                           d_options_h,
    ...
    pTextBtnCancel =
        new TextButtonClass(BUTTON_CANCEL, TXT_WOL_CANCELGAME, TPF_BUTTON, d_cancel_x, d_cancel_y, d_cancel_w);
    pTTipCancel =
        new ToolTipClass(pTextBtnCancel, TXT_WOL_TTIP_CANCELGAME, d_cancel_x + d_cancel_w / 2, d_cancel_y - 6);

    if (bHost) {
        pTextBtnAcceptStart = new TextButtonClass(
            BUTTON_ACCEPTSTART, TXT_WOL_STARTBUTTON, TPF_BUTTON, d_accept_x, d_accept_y, d_accept_w);
        pTTipAcceptStart =
            new ToolTipClass(pTextBtnAcceptStart, TXT_WOL_TTIP_START, d_accept_x + d_accept_w / 2, d_accept_y - 6);
    } else {
        pTextBtnAcceptStart = new TextButtonClass(
            BUTTON_ACCEPTSTART, TXT_WOL_ACCEPTBUTTON, TPF_BUTTON, d_accept_x, d_accept_y, d_accept_w);
        pTTipAcceptStart =
            new ToolTipClass(pTextBtnAcceptStart, TXT_WOL_TTIP_ACCEPT, d_accept_x + d_accept_w / 2, d_accept_y - 6);
    }

    pTextBtnAction = new TextButtonClass(BUTTON_ACTION, TXT_WOL_ACTION, TPF_BUTTON, d_action_x, d_action_y, d_action_w);
    pTTipAction =
        new ToolTipClass(pTextBtnAction, TXT_WOL_TTIP_ACTION, d_action_x + d_action_w / 2, d_action_y - 6, true);
    ...
    //	pStaticDescrip is no longer used - can't get the bloody thing to clip text. You'd think a StaticButton control
    //would.
    ...
    pStaticCredits =
        new StaticButtonClass(0, "         ", TPF_TEXT, d_credits_x + d_credits_w + 2 * RESFACTOR, d_credits_y);
    pStaticAIPlayers =
        new StaticButtonClass(0, "   ", TPF_TEXT, d_aiplayers_x + d_aiplayers_w + 2 * RESFACTOR, d_aiplayers_y);

    Fancy_Text_Print("", 0, 0, 0, 0, TPF_TEXT);
    pDropListHouse = new DropListClass(BUTTON_HOUSE,
                                       szHouseBuffer,
                                       sizeof(szHouseBuffer),
                                       TPF_TEXT,
                                       d_house_x,
                                       d_house_y,
                                       d_house_w,
    ...
    pCheckAftermathUnits = new BigCheckBoxClass(BUTTON_AFTERMATHUNITS,
                                                d_amunits_x,
                                                d_amunits_y,
                                                d_amunits_w,
                                                d_amunits_h,
...
#define TABSPACING 38
    pShpBtnScenarioRA = new ShapeButtonClass(
        BUTTON_SCENARIO_RA, MFCD::Retrieve("tabra.shp"), d_scenariolist_x, d_scenariolist_y - d_tab_h);
    pShpBtnScenarioCS = new ShapeButtonClass(
        BUTTON_SCENARIO_CS, MFCD::Retrieve("tabcs.shp"), d_scenariolist_x + TABSPACING, d_scenariolist_y - d_tab_h);
    pShpBtnScenarioAM = new ShapeButtonClass(
        BUTTON_SCENARIO_AM, MFCD::Retrieve("tabam.shp"), d_scenariolist_x + TABSPACING, d_scenariolist_y - d_tab_h);
    ...
    if (pWO->GameInfoCurrent.GameKind == CREATEGAMEINFO::CSGAME
        ...
        || (pWO->GameInfoCurrent.GameKind == CREATEGAMEINFO::RAGAME && Is_Aftermath_Installed()
            && !pWO->GameInfoCurrent.bTournament))
    ...
    pShpBtnScenarioUser = new ShapeButtonClass(
        BUTTON_SCENARIO_USER, MFCD::Retrieve("tabus.shp"), iScenarioUserTabPos, d_scenariolist_y - d_tab_h);
            ...
            for (i = 0; i < Session.Scenarios.Count(); i++) {
                ...
                if (pMMission->Get_Official()) {
                    if (Is_Mission_Counterstrike((char*)(Session.Scenarios[i]->Get_Filename()))) {
                        ...
                        //); 	If this is not an Aftermath game channel, we must filter out any AM maps that have 	special
                        //AM units on them.
                        if (pWO->GameInfoCurrent.GameKind == CREATEGAMEINFO::AMGAME
    ...
    while (bProcess) {
        ...
        if (HousePrevious != Session.House) {
            if (bHost) {
                ...
                if (pUserHost) //	Else we have not received the user list yet and don't know who the host is.
                               //	We'll keep trying this until we get a host - HousePrevious keeps us triggering until
                               //then.
                {
                    ...
                    //	Set house in our own list. This is fine because we know that the change must be affirmed by the
                    //host.
                    SetPlayerHouse(pWO->szMyName, Session.House);
                    HousePrevious = Session.House;
                }
            ...
            */
            ...
            if (display >= REDRAW_BACKGROUND) {
                ...
                if (bHost)
                    Fancy_Text_Print(TXT_SCENARIOS,
                                     d_scenariolist_x + d_scenariolist_w,
                                     d_scenariolist_y - 12,
                                     scheme,
                                     TBLACK,
                                     TPF_TYPE | TPF_RIGHT);
                //				else
                //					Fancy_Text_Print( TXT_SCENARIO_COLON, d_scenariolist_x + (d_scenariolist_w / 2),
                ...
                //	Game kind.
                Fancy_Text_Print(
                    szGameKind, d_gamekind_x, d_gamekind_y - iGameInfoSpacingY * 1, scheme, TBLACK, TPF_TYPE);
                //	Game kind icon.
                ...
                if (pWO->GameInfoCurrent.bPrivate) {
                    ...
                    Fancy_Text_Print(szPrivatePassword,
                                     d_gamekind_x + iGameInfoSecondColumnX,
                                     d_gamekind_y + iGameInfoSpacingY * 2,
                                     scheme,
                                     TBLACK,
                                     TPF_TYPE);
                    CC_Draw_DIB(pWO->DibIconInfos[DIBICON_PRIVATE].pDIB,
                                d_gamekind_x + iGameInfoSecondColumnX - 16,
                                d_gamekind_y + iGameInfoSpacingY * 2 - 2,
                                100,
                                WINDOW_MAIN);
                }
                //	"Scenario:" - scenario name is drawn separately.
                // Fancy_Text_Print( TXT_SCENARIO_COLON, d_gamekind_x, d_gamekind_y - iGameInfoSpacingY, scheme, TBLACK,
                // TPF_TYPE );
            }
            ...
            if (display >= REDRAW_PARMS) {
                ...
                Conquer_Clip_Text_Print(
                    szScenarioNameDisplay, d_gamekind_x, d_gamekind_y, scheme, TBLACK, TPF_TYPE, d_gamekind_w);
        ...
        if (pToolTipHead) {
            ...
            if (pToolTipHit == pToolTipHitLast) {
                if (pToolTipHit && !pToolTipHit->bShowing && ::timeGetTime() > timeToolTipAppear
            ...
            if (!bWaitingToStart) {
                ...
                if (!bHost) {
                    if ((Get_Mouse_X() >= d_count_x && Get_Mouse_X() <= d_count_x + d_count_w
                        ...
                        || (Get_Mouse_X() >= d_scenariolist_x && Get_Mouse_X() <= d_scenariolist_x + d_scenariolist_w
                            && Get_Mouse_Y() >= d_scenariolist_y
                            && Get_Mouse_Y() <= d_scenariolist_y + d_scenariolist_h)) {
                        // Session.Messages.Add_Message(NULL, 0, (char *)Text_String(TXT_ONLY_HOST_CAN_MODIFY),
                        // PCOLOR_BROWN, TPF_TEXT, 1200);
                        WOL_PrintMessage(
                            *pILDisc, Text_String(TXT_ONLY_HOST_CAN_MODIFY), WOLCOLORREMAP_LOCALMACHINEMESS);
                        Sound_Effect(WOLSOUND_ERROR);
                        if (display < REDRAW_MESSAGE)
        ...
        case (BUTTON_HOUSE | KN_BUTTON):
            ...
            /*
                            ...
                            {
                                Session.House = HouseTypeClass::From_Name( pDropListHouse->Current_Item() );	//
            ...
            */
            ...
            else if (display < REDRAW_BACKGROUND)
        ...
        case (BUTTON_COUNT | KN_BUTTON):
            Session.Options.UnitCount = pGaugeCount->Get_Value() + SessionClass::CountMin[Session.Options.Bases];
            bInformParamChange = true;
            if (display < REDRAW_PARMS)
        ...
        case (BUTTON_AIPLAYERS | KN_BUTTON):
            ...
            //				if (Session.Options.AIPlayers+Session.Players.Count() > Rule.MaxPlayers) {	// if it's pegged, max
            //it out
        ...
        case (BUTTON_ACCEPTSTART | KN_BUTTON): //	'Accept' or 'Start Game' button.
            if (!bHost) {
                ...
                {
                    //	Set ourself as accepted. We want to do this immediately so the user has feedback for pressing
                    //button. 	Besides, the host is guaranteed to allow this and set us to "accepted", unless it is
                    //simultaneously 	sending us a new update we haven't received yet. If this is the case, we will get
                    //that update in a 	second and will set ourselves to unaccepted, which will match the unaccepted
                    //state on the server. 	Upshot - We can ignore messages from the server telling us that we, ourself,
                    //accepted.
                    if (pToolTipHitLast && pToolTipHitLast->bShowing)
                        pToolTipHitLast->Unshow();
                    pTextBtnAcceptStart->Disable();
    ...
    if (bBind) {
        ...
        if (bHost) {
            ...
            case SCENARIO_CS: //	pWO->GameInfoCurrent.GameKind must be CREATEGAMEINFO::CSGAME.
                ...
                break;
            case SCENARIO_AM: //	pWO->GameInfoCurrent.GameKind must be CREATEGAMEINFO::AMGAME, or RAGAME with AM
                              //installed.
                if (!pWO->GameInfoCurrent.bTournament) {
                    pShpBtnScenarioUser->Add_Tail(*commands);
                    pShpBtnScenarioUser->DRAWTABDOWN;
...
void WOL_GameSetupDialog::ProcessGuestRequest(User* pUser, const char* szRequest)
{
    ...
    case WOL_GAMEOPT_REQCOLOR: {
        ...
        if (pILPlayers->FindColor(&ColorRemaps[ColorDesired == PCOLOR_DIALOG_BLUE ? PCOLOR_REALLY_BLUE : ColorDesired])
            == -1) {
            //	Color is available.
    ...
    case WOL_GAMEOPT_REQSTART:
        //	Does Param ID of accept request match the last param change ID sent? See notes at top.
        if (atoi(szRequest) == nHostLastParamID) //	Otherwise ignore - it's old and we don't care. (Incredibly unlikely
                                                 //to happen, actually.)
        {
    ...
    case WOL_GAMEOPT_INFGO:
        ...
        //	This is the new method. Avoids apparent "private messages/gameopts are getting delayed" problem in
        //chatserver... 	(I don't want to end up leaving the channel before guests get my go message.)
...
void WOL_GameSetupDialog::ProcessInform(char* szInform)
{
    ...
    if (!bHost) {
        ...
        case WOL_GAMEOPT_INFHOUSE: //	Note: In theory, I could ignore this if it refers to me. I've already set my own
                                   //house.
        {
        ...
        case WOL_GAMEOPT_INFCANCELSTART:
            ...
            display = REDRAW_ALL;
            //	If we are in a modal dialog, we must have arrived here through Call_Back()'s PumpMessages. Set global
            //that will 	force a cancel out of the dialog.
            if (pWO->bPump_In_Call_Back)
...
bool operator==(const GAMEPARAMS& gp1, const GAMEPARAMS& gp2)
{
    return gp1.GPacket == gp2.GPacket && gp1.bAftermathUnits == gp2.bAftermathUnits
           && gp1.bSlowUnitBuildRate == gp2.bSlowUnitBuildRate;
}
...
bool operator==(const GlobalPacketType& gp1, const GlobalPacketType& gp2)
{
    if (strcmp(gp1.ScenarioInfo.Scenario, gp2.ScenarioInfo.Scenario) != 0)
        return false;
    if (strcmp(gp1.ScenarioInfo.ShortFileName, gp2.ScenarioInfo.ShortFileName) != 0)
        return false;
    //	Digest is not null-terminated...
    ...
    if (gp1.ScenarioInfo.FileLength == gp2.ScenarioInfo.FileLength
        ...
        && gp1.ScenarioInfo.Credits == gp2.ScenarioInfo.Credits && gp1.ScenarioInfo.IsBases == gp2.ScenarioInfo.IsBases
        ...
        && gp1.ScenarioInfo.AIPlayers == gp2.ScenarioInfo.AIPlayers && gp1.ScenarioInfo.Seed == gp2.ScenarioInfo.Seed
...
void WOL_GameSetupDialog::ClearAllAccepts()
{
    ...
    for (int i = 0; i < pILPlayers->Count(); i++) {
        ...
        if (pUser
            && !(pUser->flags & CHAT_USER_CHANNELOWNER)) //	pUser null if this is an "early insertion" entry on startup
...
bool WOL_GameSetupDialog::bNeedScenarioDownload()
{
    ...
    if (!bHost) {
        if (Find_Local_Scenario(Session.Options.ScenarioDescription,
            ...
            */
            //			debugprint( "Requesting download\n" );		//	ajw  Shouldn't be happening with am maps when i have
            //am...?
...
void WOL_GameSetupDialog::HostSaysGo()
{
    ...
    while (pUser) {
        ...
        sprintf(
            ...
            " %s %02u",
            (char*)pUser->name,
            Color); //	What if player left just now, and got removed from list. Ok to continue and fail on game start?
...
void WOL_GameSetupDialog::TriggerGameStart(char* szGoMessage)
{
    ...
    // WChatHWND = (HWND) WWGetPrivateProfileInt("Internet", "HWND", (int)FindWindow("OWL_Window", "Westwood Chat"),
    // buffer);

    //	gotit	Session.Options.AIPlayers = WWGetPrivateProfileInt("Options", "AI", 0, buffer);		//Number of AI
    //players
    ...
    //	Now do whatever we've left out that the horrific Net_Fake_New_Dialog() and Net_Fake_Join_Dialog() used to do for
    //us...
    ...
    //	I will set "ID" based on a player's color, though it seems unclear if this is even used in the game, or what it
    //should be.
    ...
    //	Remember, all other data should already be sync'ed because it has been implemented in such a way that changes
    //would 	cause "unacceptedness" of guests to occur.
    ...
    for (iItem = 0; iItem < pILPlayers->Count(); iItem++) {
        //	The following is not very efficient, but doesn't have to be. Better in this case to keep it clear and
        //simple.
        ...
        if (strcmp(szPlayerName, pWO->szMyName) != 0 && GetPlayerColor(szPlayerName) != PCOLOR_NONE) {
            ...
            //	If player is the game host, set HostAddress. This global is used when downloading scenarios; who knows
            //where else.
            ...
            if (pUser && pUser->flags & CHAT_USER_CHANNELOWNER) {
                ...
                //				debugprint( "Host, ip %i.%i.%i.%i.%i.%i\n", nodexxx[0], nodexxx[1], nodexxx[2],
                nodexxx[3], nodexxx[4], nodexxx[5] );
                */
            }
            /*
                        else
    ...
    if (!bHost) //	Else ScenarioIndex is already set.
    {
        if (bRequestedScenarioDownload) {
            ...
            if (bSpecialAftermathScenario(Session.Options.ScenarioDescription)) {
                //	Shouldn't ever happen. We should never have the opportunity to ask for one of these maps to be
                //downloaded.
                bExitForGameTrigger = false;
                *szTriggerGameStartInfo = 0;
            ...
            /*	This is how the same code existed previously. Insufficient because Description may match on many
               scenarios. Session.Options.ScenarioIndex = -1; for (int i = 0; i < Session.Scenarios.Count(); i++) { if
               (!strcmp (Session.Scenarios[i]->Description(), Session.Options.ScenarioDescription) ){
