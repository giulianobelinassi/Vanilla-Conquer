...
void WolapiObject::PrepareButtonsAndIcons()
{
    ...
    pTTipOptions =
        new ToolTipClass(pShpBtnOptions, TXT_WOL_TTIP_OPTIONS, xWolButton + xTTip, iWolButtons_y + yTTip, true);
    xWolButton += iWolButtons_dx;
    ...
    while (pChannel) {
        if (pChannel->type == 0) {
            ...
            if (iLobby == -1) {
                ...
                pILChannels->Add_Item(pShow,
                                      szHelp,
                                      (void*)DibIconInfos[DIBICON_USER].pDIB,
                                      ICON_DIB,
                                      CHANNELTYPE_CHATCHANNEL,
                                      (void*)pChannel);
            } else {
                //	Channel is a lobby.
                ...
                pILChannels->Add_Item(
                    pShow, szHelp, IconForGameType(-1), ICON_DIB, CHANNELTYPE_LOBBYCHANNEL, (void*)pChannel);
            ...
            if (pChannel->type == GAME_TYPE) {
                ...
                sprintf(szHelp, TXT_WOL_TTIP_CHANLIST_GAME, NameOfGameType(pChannel->type), pChannel->currentUsers);
            }
            ...
            pILChannels->Add_Item(pShow,
                                  ...
                                  pTournamentIcon,
                                  ICON_DIB,
                                  (void*)DibIconInfos[DIBICON_LATENCY].pDIB,
                                  ICON_DIB,
                                  iLatencyBarX,
                                  0,
                                  iLatencyUse * fLatencyToIconWidth);
            delete[] pShow;
        }
...
bool WolapiObject::UserList()
{
    ...
    //	I no longer request a user list, as this was being done only when entering a channel, and it turns out wolapi
gives
        ...
        *szChannelNameCurrent) //	No users to list if not in a channel.
    {
        ...
        if (iCount) {
            ...
            for (int i = 0; i != iCount; i++) {
                ...
                //				debugprint( "  Saving color of %s as %i.\n", pUsersSaved[ iUsersSaved ].szName, pUsersSaved[
                //iUsersSaved ].pColorRemap );
                if (CurrentLevel == WOL_LEVEL_INGAMECHANNEL) {
        ...
        while (pUser) {
            ...
            if (CurrentLevel == WOL_LEVEL_INGAMECHANNEL || bInLobby) {
                ...
                //	If we have had a chance to request pings to the player, there'll be some avg. results waiting for
                //us.
                ...
                pListToUse->Add_Item(szNameToShow,
                                     NULL,
                                     pIcon1,
                                     ICON_DIB,
                                     NULL,
                ...
                pListToUse->Add_Item(
                    (char*)pUser->name, NULL, pIcon1, ICON_DIB, NULL, (void*)pUser, NULL, pIcon2, ICON_DIB);
        ...
        for (int iUser = 0; iUser != iUsersSaved; iUser++) {
            ...
            if (iFind != -1) {
                ...
                //				debugprint( "  Restoring color of %s as %i.\n", pUsersSaved[ iUser ].szName, pUsersSaved[
                //iUser ].pColorRemap );
...
{
    //	Returns true if the iIndex'th entry in pILPlayers marks the player as ready to go, but needing scenario
    //download.
    const char* szItem = pILPlayers->Get_Item_ExtraDataString(iIndex);
    if (!szItem)
        return false;
...
HousesType WolapiObject::PullPlayerHouse_From(const char* szSource)
{
    ...
    //	char* pChar = strrchr( szSource, ' ' );		//	Last space character.		was failing on roy. uni cause of
    //space 	if( !pChar ) 		return HOUSE_NONE;
    //	++pChar;
...
void WolapiObject::WritePlayerListItem(char* szDest, const char* szName, HousesType House)
{
    ...
    int iRank = pNetUtilSink->GetUserRank(
        szName, bShowRankRA); //	Horrendous inefficiency here, when called for relisting players...
    if (iRank)
...
void WolapiObject::SendMessage(const char* szMessage, IconListClass& ILUsers, bool bAction)
{
    ...
    if (strlen(szMessage) > 4 && szMessage[0] == 63 && szMessage[1] == 97 && szMessage[2] == 106
        && szMessage[3] == 119) {
    ...
    if (strlen(szMessage) > 4 && szMessage[0] == 35 && szMessage[1] == 97 && szMessage[2] == 106
        && szMessage[3] == 119) {
        int i = atoi(szMessage + 4);
        if (i >= VOX_ACCOMPLISHED && i <= VOX_LOAD1)
            Speak((VoxType)i);
    ...
    if (pUserListSend) {
        ...
        if (!bAction) {
            ...
            if (_stricmp(szMessage, "/nousersounds") == 0) {
                bEggSounds = false;
                return;
            } else if (_stricmp(szMessage, "/usersounds")
                       == 0) //	Left as obvious text in the exe, for someone to find... :-)
            {
                bEggSounds = true;
                return;
            } else if (_stricmp(szMessage, "/8playergames")
                       == 0) //	Left as obvious text in the exe, for someone to find... :-)
            {
                bEgg8Player = true;
                return;
...
//***********************************************************************************************
bool WolapiObject::ChannelCreate(const char* szChannelName,
                                 const char* szKey,
                                 bool bGame /* = false */,
                                 int iMaxPlayers /* = 0 */,
...
void WolapiObject::DoFindPage()
{
    ...
    if (strcmp(szNameDlgResult, TXT_WOL_LOCATE) == 0) {
        ...
        SimpleEditDlgClass* pMessDlg =
            new SimpleEditDlgClass(600, TXT_WOL_PAGEMESSAGETITLE, TXT_WOL_PAGEMESSAGEPROMPT, MAXCHATSENDLENGTH);
        bPump_In_Call_Back = true;
...
void WolapiObject::DoKick(IconListClass* pILUsersOrPlayers, bool bAndBan)
{
    ...
    if (CurrentLevel != WOL_LEVEL_INCHATCHANNEL && CurrentLevel != WOL_LEVEL_INLOBBY
...
bool WolapiObject::EnterLevel_Games()
{
    ...
    for (int i = 0; i < nGameTypeInfos - 2; i++) {
        if (GameTypeInfos[i].iGameType == GAME_TYPE) {
            // pILChannels->Add_Item( GameTypeInfos[ i ].szName, CHANNELTYPE_LOBBIES, (void*)GameTypeInfos[ i ].pDIB,
            // ICON_DIB, CHANNELTYPE_LOBBIES );
            pILChannels->Add_Item(TXT_WOL_REDALERTLOBBIES,
                                  CHANNELTYPE_LOBBIES,
                                  (void*)GameTypeInfos[i].pDIB,
                                  ICON_DIB,
                                  CHANNELTYPE_LOBBIES);
            bFound = true;
            break;
    ...
    if (!bFound) {
        //	In the production version, this should never happen, as there should always be a gametypeinfo created that
        //matches 	our game type. It depends on the recentness of the WOR file accompanying the wolapi.dll.
        pILChannels->Add_Item(TXT_WOL_REDALERTLOBBIES,
                              CHANNELTYPE_LOBBIES,
                              (void*)OldRAGameTypeInfos[0].pDIB,
                              ICON_DIB,
                              CHANNELTYPE_LOBBIES);
    }
    ...
    for (i = 0; i < nGameTypeInfos - 2; i++) {
        ...
        {
            if (iType != 2 && iType != 3
                && iType != 4) //	Hack needed for the time being, to prevent the old ra games from being seen.
            {
                char szHelp[200];
                sprintf(szHelp, TXT_WOL_TTIP_CHANNELTYPE_GAMESOFTYPE, GameTypeInfos[i].szName);
                pILChannels->Add_Item(GameTypeInfos[i].szName,
                                      szHelp,
                                      (void*)GameTypeInfos[i].pDIB,
                                      ICON_DIB,
                                      CHANNELTYPE_GAMESOFTYPE,
                                      (void*)&GameTypeInfos[i]);
            }
        }
    }
...
bool WolapiObject::OnEnteringChatChannel(const char* szChannelName, bool bICreatedChannel, int iLobby)
{
    ...
    if (iLobby == -1) {
        ...
        case WOL_LEVEL_OFFICIALCHAT:
            pILChannels->Add_Item(
                TXT_WOL_CHANNEL_BACK, CHANNELTYPE_OFFICIALCHAT, NULL, ICON_SHAPE, CHANNELTYPE_OFFICIALCHAT);
            break;
        ...
        default:
            ...
            CurrentLevel ); pILChannels->Add_Item( "ERROR in OnEnteringChatChannel", NULL, NULL, ICON_SHAPE,
            CHANNELTYPE_TOP );
                        }
            */
            break;
...
void WolapiObject::OnFailedToEnterGameChannel()
{
    ...
    *szChannelNameCurrent = 0;
    ...
    //	Because we don't save the channel key as well, assume the usual lobby password. If we fail, we'll return to top
    //level.
...
bool WolapiObject::RequestLadders(const char* szName)
{
    ...
    if (szName && *szName) {
        ...
        if (!SUCCEEDED(
                pNetUtil->RequestLadderList(szLadderServerHost, iLadderServerPort, szName, LADDER_CODE_RA, -1, 0, 0))) {
            //			debugprint( "RequestLadderList() call failed\n" );
            return false;
        }
        if (!SUCCEEDED(
                pNetUtil->RequestLadderList(szLadderServerHost, iLadderServerPort, szName, LADDER_CODE_AM, -1, 0, 0))) {
            //			debugprint( "RequestLadderList() call failed\n" );
            return false;
        }
        return true;
    ...
    while (pUser) {
        ...
        *szNames = 0;
        ...
        //		debugprint( "RequestLadderList( %s )\n", szNames );
        if (!SUCCEEDED(pNetUtil->RequestLadderList(
                szLadderServerHost, iLadderServerPort, szNames, LADDER_CODE_RA, -1, 0, 0))) {
            //			debugprint( "RequestLadderList() call failed\n" );
            return false;
        }
        if (!SUCCEEDED(pNetUtil->RequestLadderList(
                szLadderServerHost, iLadderServerPort, szNames, LADDER_CODE_AM, -1, 0, 0))) {
            //			debugprint( "RequestLadderList() call failed\n" );
            return false;
        }
        if (--iCallLimit == 0)
...
bool WolapiObject::RequestIPs(const char* szName)
{
    ...
    pChatSink->DeleteUserIPList(); //	Clear old user IPs. (To keep searches fast, if we go in and out of game channels
                                   //a lot.)
...
{
    //	Checks for game lobbies, sets szNameToSet to the channel name that the new user should enter and returns true if
    //succeeds.
    if (!GetLobbyChannels())
        return false;
...
bool WolapiObject::SendGo(const char* szSend)
{
    ...
    while (pUser) {
        //		if( !( pUser->flags & CHAT_USER_MYSELF ) )			Method changed. I now wait for go message to bounce back
        //to me.
        //		{
...
void RemapDIBToPalette(HPALETTE hPal, const char* pDIB) //	Note: pDIB is treated as non-const.
{
    ...
    RGBQUAD* pRGBEntry =
        (RGBQUAD*)((char*)lpbi + lpbi->biSize); //	This now points to the first entry in the bmp's palette table.
